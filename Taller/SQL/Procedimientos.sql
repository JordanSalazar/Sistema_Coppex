...
BEGIN
    ...
    -- Construir la consulta dinámica para actualizar el atributo especificado
    SET @sql = '
    UPDATE ' + @Tabla + '
    ...
    ELSE
        PRINT 'No se encontró ningún producto con ID ' + CAST(@ID_Producto AS VARCHAR) + ' o el atributo ' + @Atributo + ' no se actualizó.';
END;
-----------------------------------------------------
-----------------------------------------------------
...
BEGIN
    ...
    -- Consulta para obtener detalles de un producto específico y su inventario
    SELECT 
        P.ID_Productos AS ID_Producto,
        P.Nombre AS Nombre_Producto,
        P.Precio_Uni AS Precio_Unitario,
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Guia_despacho (Fecha_emision, Dirección)
        VALUES (@Fecha_emision, @Direccion);

        -- Obtener el ID de la guía de despacho recién insertada
...
BEGIN
    ...
    -- Seleccionar la factura específica basada en el ID
    SELECT
        ID_Factura,
        ID_Despacho,
        Fecha_emision,
